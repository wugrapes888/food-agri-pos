...
// 成本 vs 毛利疊加柱狀圖（下方 amber=成本，上方 green=毛利）
...
function downloadTodayCsv(sales, date) {
  const PAY_LABEL = { cash: '現金', transfer: '轉帳', linepay: 'LINE Pay', ipass: 'iPass', taiwanpay: '台灣Pay' }
  const TYPE_LABEL = { preorder: '預購', walk: '散客' }
              ...
              {sales.map((tx, ti) => (
                <div key={ti} className="border-b border-gray-50 last:border-0">
                  ...
                  <div className="flex items-center gap-3 px-4 py-2.5 bg-gray-50">
                    <span className="text-xs text-gray-400 font-mono w-16 flex-shrink-0">{tx.time.slice(0,5)}</span>
                    ...
                    <span className="text-xs text-gray-500 flex-shrink-0">{PAY_LABEL[tx.paymentMethod] || tx.paymentMethod}</span>
                    {tx.staffName && <span className="text-xs text-gray-400 ml-auto flex-shrink-0">{tx.staffName}</span>}
                  </div>
                  {/* 品項 */}
                  ...
                  return (
                    <tr key={b.id ?? i} className="border-b border-gray-50 last:border-0 hover:bg-gray-50">
                      ...
                      <td className="px-3 py-1.5">
                        <input
                          type="date"
                          value={displayField(b, 'purchaseDate')}
...
function ProfitReportSection() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch(err => { if (!cancelled) { setProfitData([]); setTrendData([]); setChannelData([]); setGasError(err.message) } })
      .finally(() => { if (!cancelled) setLoading(false) })
    return () => { cancelled = true }
  }, [dim, customStart, customEnd])
                ...
                return channelData.map((c, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-3 mb-2 last:mb-0">
                      <div className="w-14 text-xs font-semibold text-gray-600 flex-shrink-0">{c.channelLabel}</div>
